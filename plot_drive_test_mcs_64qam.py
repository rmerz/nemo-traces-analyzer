...
def main(args):
    ...
    ntp.process_data(data,ntp.process_pdsch_mcs_64qam)

    column_list = ['mcs_17','mcs_18','mcs_19','mcs_20','mcs_21','mcs_22','mcs_23','mcs_24','mcs_25','mcs_26','mcs_27','mcs_28']
    ...
    for index in np.arange(0,12):
        print(index,index+17)
        print(df[['mcs_'+str(index+17)]].dropna().describe())
        plt.boxplot(df[['mcs_'+str(index+17)]].dropna().values,positions=[index+1])
    plt.ylim([0,100])
    plt.xlim([0,13])
    plt.xticks(np.arange(1,13),['MCS '+str(index+17) for index in np.arange(0,12)],rotation=45)
    plt.grid(True)
    plt.tight_layout()
